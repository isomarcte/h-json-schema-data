...
objectSchema ::
     (Applicative f)
  => (JsonObjectSchema -> f JsonObjectSchema)
  -> JsonSchema
  -> f JsonSchema
objectSchema f (ObjectSchema j) = ObjectSchema <$> f j
objectSchema _ j = pure j

objectSchema' ::
     (JsonObjectSchema -> JsonObjectSchema) -> JsonSchema -> JsonSchema
objectSchema' f jos = runIdentity $ objectSchema (pure . f) jos
